...
const dmStats = ref<any>(null), dmStatsEditing = ref(false), dmStatsDirty = ref(false)
...
async function openDm(user: UserInfo) {
  ...
  dmConvId.value = null; dmItems.value = []
...
async function loadDmTab() {
  ...
  try {
    let url = ''
    if (dmTab.value === 'messages') url = `/auth/users/${dmUser.value.id}/messages?search=${encodeURIComponent(dmSearch.value)}&limit=200`
    else if (dmTab.value === 'facts') url = `/auth/users/${dmUser.value.id}/facts?search=${encodeURIComponent(dmSearch.value)}&limit=50`
    else if (dmTab.value === 'summaries') url = `/auth/users/${dmUser.value.id}/summaries?search=${encodeURIComponent(dmSearch.value)}&limit=50`
...
function adjustStat(key: string, delta: number) {
  if (!dmStats.value) return
  dmStats.value[key] = Math.max(0, (dmStats.value[key] || 0) + delta)
  dmStatsDirty.value = true
}
async function saveDmStats() {
  if (!dmStats.value) return
  try {
    await apiClient.put(`/auth/users/${dmUser.value!.id}/stats`, { conversations: dmStats.value.conversations, messages: dmStats.value.messages, facts: dmStats.value.facts, summaries: dmStats.value.summaries })
    dmStatsEditing.value = false; dmStatsDirty.value = false
  } catch {}
}
async function resetDmStats() {
  try {
    await apiClient.put(`/auth/users/${dmUser.value!.id}/stats`, { reset: true })
    await loadDmStats()
    dmStatsEditing.value = false; dmStatsDirty.value = false
  } catch {}
}
  ...
  <div class="h-full overflow-y-auto p-6 space-y-6">
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        <div class="relative"><svg class="absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-foreground" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
          <input v-model="searchQuery" placeholder="搜索用户..." class="h-9 w-44 rounded-lg border border-input bg-background pl-8 pr-3 text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" /></div>
        ...
        <button @click="showCreate = !showCreate" class="inline-flex items-center gap-1.5 rounded-lg bg-primary text-primary-foreground px-3 py-2 text-sm font-medium hover:bg-primary/90 transition-colors"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14"/><path d="M12 5v14"/></svg>添加用户</button>
      </div>
    </div>
      ...
      <div v-if="showDm && dmUser" class="fixed inset-0 z-[9999] flex items-start justify-center bg-black/40 backdrop-blur-sm p-4 pt-12 overflow-y-auto" @click="closeDm">
        <div class="bg-card border border-border rounded-xl w-full max-w-2xl shadow-xl animate-scale-in overflow-hidden" @click.stop>
          ...
          <div class="p-5 max-h-[55vh] overflow-y-auto">
                ...
                >
                  ...
                  <div class="flex items-center justify-between gap-2 flex-1 min-w-0">
                    <div class="flex items-center gap-2 min-w-0">
                      ...
                      <span class="text-xs font-medium truncate">{{ g.msgs[0]?.content?.slice(0,60) || '对话' }}</span>
                    </div>
                    <span class="text-[10px] text-muted-foreground shrink-0">{{ g.count }} 条 · {{ g.latest.toLocaleDateString() }}</span>
                  </div>
            ...
            <!-- Summaries tab -->
            <div v-else-if="dmTab === 'summaries'" class="space-y-2">
              <div v-if="dmItems.length === 0" class="text-center py-6 text-xs text-muted-foreground">暂无摘要</div>
              <div v-for="s in dmItems" :key="s.id" class="rounded-lg border border-border p-2.5">
                <div v-if="dmEditId === s.id">
                  <textarea v-model="dmEditText" class="w-full rounded border border-input bg-background p-2 text-xs min-h-[60px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" />
                  <div class="flex gap-1 mt-1.5 justify-end">
                    <button @click="saveEditItem(s, 'summary', `/auth/users/${dmUser!.id}/summaries/${s.id}`)" class="rounded bg-primary text-primary-foreground px-2 py-0.5 text-xs">保存</button>
                    <button @click="cancelEdit" class="rounded border border-border px-2 py-0.5 text-xs hover:bg-muted">取消</button>
                  </div>
                </div>
                <template v-else>
                  <div class="flex items-center justify-between gap-2">
                    <span class="text-[10px] text-muted-foreground">{{ new Date(s.timestamp).toLocaleString() }}{{ s.conv_id ? ' · conv_' + s.conv_id.slice(0,8) : '' }}</span>
                    <div class="flex gap-1 shrink-0">
                      <button @click="startEdit(s, 'summary')" class="text-[9px] text-muted-foreground hover:text-foreground px-1">编辑</button>
                      <button @click="requestDeleteItem(`/auth/users/${dmUser!.id}/summaries/${s.id}`, '此摘要')" class="text-[9px] text-destructive hover:text-destructive/80 px-1">删除</button>
                    </div>
                  </div>
                  <p class="text-xs text-foreground/85 mt-1">{{ s.summary }}</p>
                </template>
              </div>
            </div>
              ...
              <div v-else class="space-y-4">
                <div class="grid grid-cols-2 gap-3">
                  <div v-for="key in ['conversations','messages','facts','summaries']" :key="key" class="rounded-lg bg-muted/30 p-3 text-center">
                    <div v-if="dmStatsEditing" class="flex items-center justify-center gap-1">
                      <button @click="adjustStat(key, -1)" class="w-6 h-6 rounded bg-muted-foreground/10 hover:bg-muted-foreground/20 text-xs font-bold transition-colors">−</button>
                      <span class="text-lg font-bold text-foreground min-w-[2ch] text-center">{{ dmStats[key] }}</span>
                      <button @click="adjustStat(key, 1)" class="w-6 h-6 rounded bg-muted-foreground/10 hover:bg-muted-foreground/20 text-xs font-bold transition-colors">+</button>
                    </div>
                    <template v-else><p class="text-2xl font-bold text-foreground">{{ dmStats[key] }}</p></template>
                    <p class="text-[10px] text-muted-foreground mt-0.5">{{ { conversations:'对话', messages:'消息', facts:'记忆事实', summaries:'摘要' }[key] }}</p>
                  </div>
                </div>
                <div v-if="dmStatsEditing" class="flex gap-2">
                  <button @click="saveDmStats" :disabled="!dmStatsDirty" class="flex-1 rounded-lg bg-primary text-primary-foreground px-3 py-1.5 text-xs font-medium hover:bg-primary/90 transition-colors disabled:opacity-50">保存修改</button>
                  <button @click="resetDmStats" class="rounded-lg border border-border px-3 py-1.5 text-xs hover:bg-muted transition-colors">重置</button>
                </div>
                <div v-else class="flex gap-2">
                  <button @click="dmStatsEditing = true; dmStatsDirty = false" class="flex-1 rounded-lg bg-primary text-primary-foreground px-3 py-1.5 text-xs font-medium hover:bg-primary/90 transition-colors">编辑数据</button>
                  <button @click="requestClearUserData()" class="rounded-lg border border-destructive/30 text-destructive px-3 py-1.5 text-xs hover:bg-destructive/10 transition-colors">清空数据</button>
                </div>
              </div>
            </div>
          </div>
