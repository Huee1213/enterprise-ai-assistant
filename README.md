...
- **🔧 智能体配置管理** — 分区 Tab 可视化配置（LLM 模型 / 向量嵌入 / 检索参数 / Agent 行为），各分区独立「保存修改」、独立恢复默认、撤销未保存修改（分区单撤 + 全局一键撤销，纯本地回滚）；也可一键恢复全局默认；每项配置带功能描述（悬停/点击问号或文字即显示说明）；保存后立即生效（无需重启），工具开关/检索阈值/系统提示词/最大轮次均在运行时强制
