...
- **🧬 本地嵌入模型管理** — 本地模型支持从 fastembed 支持列表一键选择（含维度/体积/已缓存标识，`GET /api/agent/config/embedding/models`）；可配置下载提供商（默认 https://hf-mirror.com）；保存时自动下载并显示实时进度，失败自动回退官方 Hub 或保留原配置；成功后自动重建 Milvus 向量库并重索引全部文档，Agent 无需重启即可检索（`POST .../embedding/prepare`、`POST .../embedding/apply`）
...
- **📱 响应式布局 + 🌙 深色模式 + ✨ 过渡动画** — 桌面端侧栏可缩边（仅图标），移动端滑出式侧栏；持久主题切换 + 系统偏好检测；路由切换并发淡入淡出（无遮罩不白屏）、弹窗进出缩放淡变动画，全站页面与组件均带平滑过渡
...
# 系统管理员账号（super_admin）由 init_db 自动创建：
#   用户名: admin  工号: ADM-000001  初始密码: Admin@123  显示名: 系统管理员
# ⚠️ 请首次登录后在 系统-用户管理 中立即修改初始密码
```
...
打开浏览器访问 `http://localhost`，使用系统管理员账号（`admin` / `Admin@123`）登录后即可管理文档和用户。
1. 点击右上角 **后台** → **文档管理**
